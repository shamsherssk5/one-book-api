...
module.exports = () => {
    ...
    app.post('/signup', (req, res) => {
        ...
        DBConnection.executeQuery(checkQuery, (err, result) => {
            if (parseInt(result[0].exist) === 0) {
                ...
                var query = "INSERT INTO `users` (`first_name`, `last_name`, `mobile`, `email`, `password`, `userName`, `role`) VALUES ('" + req.body.first_name + "', '" + req.body.last_name + "', '" + req.body.mobile_num + "', '" + req.body.email + "', '" + hash + "',  '" + req.body.first_name + " " + req.body.last_name + "', 'admin')";
    ...
    app.post('/signin', (req, res) => {
        ...
        var checkQuery="select u.password, u.userName,u.userID,u.userAvatar, u.role, u.orgID,l.timeZone,l.timeFormat,l.dateFormat from users u, location l where l.orgID in (u.orgID,0) and u.email='" + req.body.user_email + "' ORDER by l.orgID DESC LIMIT 1"
        DBConnection.executeQuery(checkQuery, (err, result) => {

            if (result && result.length > 0) {

                let check = bcrypt.compareSync(req.body.user_password, result[0].password);
                ...
                if (check) {
                    ...
                    ResponseMaker.customResponse(res, { "message": "Invalid Credentials" });
                }
    ...
    app.post('/sendLink', (req, res) => {
        var checkQuery = "select u.userName from users u where u.email='" + req.body.email + "'";
    ...
    app.post('/resetPassword', (req, res) => {
        ...
        var checkQuery = "select u.userName from users u where u.email='" + decode.email + "'";

        DBConnection.executeQuery(checkQuery, (err, result) => {

            if (result && result.length > 0) {
                const hash = bcrypt.hashSync(req.body.password, saltRounds);
                var query="update users u set password='"+ hash +"' where u.email='" + decode.email + "'";
                DBConnection.executeQueryAndSendResponse(res, query);
